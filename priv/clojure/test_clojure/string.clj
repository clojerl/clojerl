...
(deftest nil-handling
  (are [f args] (thrown? :error (apply f args))
       s/reverse [nil]
...
(defn string-writer [s]
  (new erlang.io.StringWriter s))

(deftest char-sequence-handling
  (are [result f args] (let [[^CharSequence s & more] args]
                         (= result (apply f (string-writer s) more)))
       "paz" s/reverse ["zap"]
       "foo:bar" s/replace ["foo-bar" \- \:]
       "ABC" s/replace ["abc" #"\w" s/upper-case]
       "faa" s/replace ["foo" #"o" (string-writer "a")]
       "baz::quux" s/replace-first ["baz--quux" #"--" "::"]
       "baz::quux" s/replace-first ["baz--quux" (string-writer "--") (string-writer "::")]
       "zim-zam" s/replace-first ["zim zam" #" " (string-writer "-")]
...
(deftest t-starts-with?
  (is (s/starts-with? (string-writer "clojure west") "clojure"))
  (is (not (s/starts-with? (string-writer "conj") "clojure"))))

(deftest t-ends-with?
  (is (s/ends-with? (string-writer "Clojure West") "West")
  (is (not (s/ends-with? (string-writer "Conj") "West")))))

(deftest t-includes?
  (let [sb (string-writer "Clojure Applied Book")]
