...
PID=`erl -pa ebin -pa deps/*/ebin -sname clojerl-flame -s clojerl -noshell -eval "eflame:apply(normal_with_children, \"$TRACE_FILE\", clj_compiler, compile_file, [<<\"$FILE\">>]), io:format(\"~p~n\", [self()]), erlang:halt(0)."`
